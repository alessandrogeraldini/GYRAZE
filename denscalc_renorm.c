...
void densfinorb(double Te, double alpha, int size_phigrid, int *size_ngrid, double* n_grid, double *x_grid, double* phi_grid, double charge, double **FF, double *mumu, double *UU, int sizemumu, int sizeUU, double grid_parameter, double *flux, int zoomfactor, double stopdens, double phi_DSbump, double *vy_op, double *chiMax_op, double *dmudvy_op) {
	...
	double deltax, deltax_inf, deltaE = 0.1;
	double *chiinf, *muinf, **vxinf;
	double *xx, *phi, *phip, **chi;
	...
	double **Uperp, ***vx, *chiMax, *chimpp, *chimin, oorbintgrd, oorbintgrdantycal, oorbintgrdBohm;
	...
	printf("Te= %f\tcharge = %f\n", Te, charge);
	printf("phi[0] = %f\tphi_grid[0] = %f\n", phi[0], phi_grid[0]);

	// Introduce a cap in energy (U, Uperp) high enough that we can safely assume F = 0
	Ucap = 18.0 + 6.0*Te;
	if (bilin_interp(0.0, Ucap, FF, mumu, UU, sizemumu, sizeUU, -1, -1) > 1e-9) printf("ERROR in densfinorb_renorm.c: increase Ucap please\n");
	else if (bilin_interp(Ucap, 0.0, FF, mumu, UU, sizemumu, sizeUU, -1, -1) > 1e-9) printf("ERROR in densfinorb_renorm.c: increase Ucap please\n");
	else printf("Ucap has been checked to be large enough\n");

	// Evaluate integrals of distribution function at infinity
	fluxinfintgrd = fluxinf1 = fluxinf = densinf = densinf1 = Chodura2 = Chodura1 = Chodura = 0.0;
	du = 0.01;
	...
	*flux = fluxinf/densinf;
	Chodura /= densinf;
	printf("densinf = %f\tfluxinf = %f\tChodura = %f\n", densinf, *flux, Chodura);
	...
	*/
	...
	Because I am comparing neighbouring values of x to find a maximum, I need to consider two distinct cases.The first one is treated in the if loop below, which the program should enter only if chi is decreasing at x=0. Implying that chi(x=0) is an effective potential maximum. The second one is the else if loop after that, which finds maxima of chi that are stationary points by comparing the value of the function before and after each point. Note that because we compare the function at a point with the function at points before and after, the point we consider at every iteration step in the index i is indexed (i-1), compared with (i-2) and i.  */
			if ( (i==1) && (chi[j][i] < chi[j][i-1]) ) {	
				...
				if (phi_DSbump > 1e-6) {
					...
					//printf("ever here? kdrop = %d\tphi_DSbump = %f\tdeltaE = %f\n", kdrop[j], phi_DSbump, deltaE);
				}
				else {
					chiMax[j] = chi[j][0];
					kdrop[j] = 0;
	...
	// crossed_min and crossed_max let the code know whether we go up or down the effective potential well. We are starting to go down!
				...
				//if (phi_DSbump > chiMax[j] - chi[j][0] + 1e-7) {
				//	kdrop[j] = ceil((phi_DSbump - (chiMax[j]-chi[j][0]))/deltaE); 
				//	deltaE = (phi_DSbump - (chiMax[j]-chi[j][0]))/kdrop[j];
				//}
				//else kdrop[j] = 0;
				//for (k=0; k<=kdrop[j]; k++) {
				//	Uperp[j][k] = chi[j][0] - k*deltaE;
				//		
				//}
			}
	...
	// temporary
	muopen[maxj-1] = 5000.0;
	chiMopen[maxj-1] = 5000.0; 
		...
		for (j=0; j<sizexbar; j++) { //*
			...
			if (j == jmopen[i]) {
				...
				for (l=0; l < sizeU; l++) {
					...
					if (i==0) 
						oorbintgrdantycal = sqrt(2.0*alpha*vz*M_PI*xbar[j])*(U-chiMax[j])*exp(-U)/pow(M_PI, 1.5);  // CHECK NORMALIZATION
				...
				for (l=0; l < sizeU; l++) {
					...
					if ( (U >= mu[j][0]) && (U - 0.5*vz*vz + 0.5*(vz-dvzopen)*(vz-dvzopen) < mu[j][0]) ) {
						...
						oorbintgrdBohmold = (1.0/vx0open - 1.0/sqrt(vx0open*vx0open + 2.0*alpha*sqrt(2.0*(mu[j][0] - chiMax[j]))*openorbit[j]))*Fopen;
						oorbintgrdxbarold = xbar[j]*(1.0/vx0open - 1.0/sqrt(vx0open*vx0open + 2.0*alpha*sqrt(2.0*(mu[j][0] - chiMax[j]))*openorbit[j]))*Fopen;
						oorbintgrdsquareold = (1.0/8.0)*(1.0/pow(vx0open, 3.0) - 1.0/pow((vx0open*vx0open + 2.0*alpha*sqrt(2.0*(mu[j][0] - chiMax[j]))*openorbit[j]), 1.5))*Fopen;
						...
						oorbintgrdBohm = (1.0/vx0open - 1.0/sqrt(vx0open*vx0open + 2.0*alpha*vz*openorbit[j]))*Fopen;
						oorbintgrdxbar = xbar[j]*(1.0/vx0open - 1.0/sqrt(vx0open*vx0open + 2.0*alpha*vz*openorbit[j]))*Fopen;
						oorbintgrdsquare = (1.0/8.0)*(1.0/pow(vx0open, 3.0) - 1.0/pow((vx0open*vx0open + 2.0*alpha*vz*openorbit[j]), 1.5))*Fopen;
			...
			if ( j>=jmclosed[i] ) {	
				...
				if (j == jmclosed[i]) {
					...
					for (k=lowerlimit[j]; k<upper[j][i]+1; k++) {	
						...
						Ucrit = lin_interp(muopen, Ucritf, munew, maxj, 791);
