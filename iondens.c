...
double *openorbit, openorbitantycal, **mu, *xbar, *xbarop, **FF, *UU, *mumu, *twopimuprime, *openintegral, *xifunction;
...
char *string, *string_pot, *string_input;
// And same with the string pointers above
/////////////////////////////////////////////////////////////
/* READ INPUT FILE */
FILE *input;
...
{	
	//string_input = malloc(strlen(line_input)*sizeof(char));
    //string_input = line_input;
    //printf("The string is %s\n", string);    
	//storevalsinput = linetodata(string_input, strlen(string_input), &ncols_Umufile);
...
while (fgets(line_pot, 200, fp) != NULL)
{
	//string_pot = malloc(strlen(line_pot)*sizeof(char));
	//string_pot = line_pot;
...
passedtopM = (int*)calloc(sizexbar,sizeof(int));
passedtop = (int**)calloc(sizexbar,sizeof(int*));
...
{	chi[j] = (double*)calloc(n,sizeof(double)); // array containing chi(xbar, x) indexes j and i 
	...
	passedtop[j] = (int*)calloc(n,sizeof(int)); // see below
