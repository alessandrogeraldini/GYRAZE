...
// electron quantities
	double *ne_grid, *ne_DSgrid, *ne_DSgrid_corr_delta, *ne_DSgrid_corr_chiM, ne_inf = 0.0, elmomfluxDS0=0.0, elmomfluxDSinf=0.0;
...
// quantities related to individual ion species: dim1 is for species
	double **ni_grid, **ni_DSgrid, **ni_DS_corr, **ni_grid_corr_delta, **ni_grid_corr_chiM, ionmomfluxDS0=0.0, ionmomfluxDSinf=0.0;
...
// quantities related to overall ion properties
	double *sumni_grid, *sumni_DSgrid, *sumni_DS_corr, *sumni_grid_corr_delta, *sumni_grid_corr_chiM, sumflux_i=0.0, sumni_norm=0.0, sumQ_i=0.0;
	int size_sumnigrid;
...
*/
	...
	sumni_grid_corr_chiM = malloc(size_phigrid*sizeof(double));
	ni_grid = malloc(num_spec*sizeof(double));
	ni_grid_corr_delta = malloc(num_spec*sizeof(double));
	ni_grid_corr_chiM = malloc(num_spec*sizeof(double));
...
// gives range of values of (1/2)vx^2, upon multiplying by alpha*vz
	...
	for (n=0; n<num_spec; n++){
		...
		ni_grid_corr_chiM[n] = malloc(size_phigrid*sizeof(double));
	}
...
// gives range of values of (1/2)vx^2, upon multiplying by alpha*vz
	...
	*/
	if (TEST_EL==1) { //////// IGNORE
		...
		ne_DSgrid = malloc(size_phiDSgrid*sizeof(double)); ni_DSgrid = malloc(num_spec*sizeof(double)); ni_DS_corr = malloc(num_spec*sizeof(double)); ne_DSgrid_corr_delta = malloc(size_phiDSgrid*sizeof(double)); ne_DSgrid_corr_chiM = malloc(size_phiDSgrid*sizeof(double));
		...
		printf("size grid = %d\n", size_phiDSgrid);
		densfinorb(1.0, 1.0, alpha, size_phiDSgrid, &size_neDSgrid, ne_DSgrid, ne_DSgrid_corr_delta, ne_DSgrid_corr_chiM, x_DSgrid, phi_DSgrid, -1.0, dist_e_GK, mu_e, U_e_DS, size_mu_e, size_vpar_e, 0.0, &flux_eDS, &garbage, ZOOM_DS, -0.5, -999.9, vy_e_wall, mu_e_op, chiM_e, twopidmudvy_e, &size_op_e); 
...
*/
			...
			for (i=0; i<size_sumnigrid; i++) { // i is position index
				...
				sumni_grid_corr_chiM[i] += (nioverne[n]*ni_grid_corr_chiM[n][i]);
...
*/
	...
	for (n=0; n<num_spec; n++) {
		...
		densfinorb(TioverTe[n], lenfactor[n], alpha, size_phigrid, size_ngrid+n, ni_grid[n], ni_grid_corr_delta[n], ni_grid_corr_chiM[n], x_grid, phi_grid, ioncharge, dist_i_GK[n], mu_i[n], U_i[n], size_mu_i[n], size_U_i[n], grid_parameter, flux_i+n, Q_i+n, zoomfactor, -1.0, -999.9, vy_i_wall[n], mu_i_op[n], chiM_i[n], twopidmudvy_i[n], size_op_i+n);
		do
			size_sumnigrid = size_ngrid[n] ;
		while (size_sumnigrid > size_ngrid[n]) ;
		sumflux_i += (nioverne[n]*sqrt(TioverTe[n]/mioverme[n])*flux_i[n]);
	...
	// FULL DEBYE SHEATH SOLUTION CALCULATED WITH FINITE (DISTORTED) ELECTRON GYROORBITS
		...
		ne_DSgrid_corr_chiM = malloc(size_phiDSgrid*sizeof(double));
		...
		while ( ( ( convergence_MP <= 1) || (convergence_DS <= 1) || (convergence_j <= 1) ) && (N_DS < MAX_IT) ) {
			...
			if (gamma_DS > SMALLGAMMA) {
				...
				densfinorb(1.0, 1.0, alpha, size_phiDSgrid, &size_neDSgrid, ne_DSgrid, ne_DSgrid_corr_delta, ne_DSgrid_corr_chiM, x_DSgrid, phi_DSgrid, -1.0, dist_e_GK, mu_e, U_e_DS, size_mu_e, size_vpar_e, 0.0, &flux_eDS, &garbage, ZOOM_DS, MARGIN_DS, -999.9, vy_e_wall, mu_e_op, chiM_e, twopidmudvy_e, &size_op_e); 
...
// Calculating v_cutDS after new phiwall and new phi_MP[0] have been calculated, to ensure that the new imposed phi_DS[0] is consistent
		...
		if (gamma_DS > SMALLGAMMA) {
			densfinorb(1.0, 1.0, alpha, size_phiDSgrid, &size_neDSgrid, ne_DSgrid, ne_DSgrid_corr_delta, ne_DSgrid_corr_chiM, x_DSgrid, phi_DSgrid, -1.0, dist_e_GK, mu_e, U_e_DS, size_mu_e, size_vpar_e, 0.0, &flux_eDS, &garbage, ZOOM_DS, 0.0, -999.9, vy_e_wall, mu_e_op, chiM_e, twopidmudvy_e, &size_op_e); 
			//printf("chiM mu\n");
			for (i=0; i<size_op_e; i++) {
				vpar_e_cut_lookup[i] = sqrt(2.0*(chiM_e[i] - mu_e_op[i]));
